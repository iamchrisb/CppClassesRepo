...
/**
  *Lˆscht Werte aus dem RationalNumberArray von fromPosition bis tillPosition
  **/

void RationalNumberArray::remove(unsigned int fromPosition, unsigned int tillPosition){
    int temp = fromPosition;
    //‹berschreiben der zu lˆschenden Werte durch lineare Verschiebung der Werte nach der tillPosition
    if(fromPosition > m_size || tillPosition > m_size ) {
        ...
        m_size = m_size - count;
        //Wenn die Kapazitaet im Verh‰ltnis zur eigentlich vergebenen Grˆﬂe zu hoch ist diese verkleinern
